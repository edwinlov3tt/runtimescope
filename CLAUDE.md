...
- **Phase review (run before shipping a phase / cutting a release):** fan out **three independent review lenses** over the phase's diff/dir — **adversarial** ("assume it's wrong, prove it"), **edge-cases & gaps** (unhandled inputs + spec-coverage), **verification** (does it match stated intent; dead code; tests that don't test). Each returns severity-ranked findings with `file:line`; the orchestrator dedups by location (multi-lens agreement = higher confidence) into one ranked list. **Separate finding from fixing** — collect all findings first, review them, then run a *distinct* fix pass on the chosen ones (diagnosis-and-remediation in one breath produces motivated reasoning). The reusable harness is [`.claude/workflows/phase-review.js`](.claude/workflows/) — run it via the Workflow tool with `args` set to the scope (a dir or diff spec). Verify any Critical finding in the code yourself before acting on it.
