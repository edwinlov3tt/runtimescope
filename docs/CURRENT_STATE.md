...
**Snapshot date:** 2026-06-01 (M7 Rust cutover — structural cutover done; npm publish pending)
**Snapshot commit:** `5fc3e34` (v0.11.0 version bump) on top of `ab1095d` (M7 cutover: Node collector/mcp-server/cli deleted). See [`reports/m7-rust-cutover-completion-report.md`](./reports/m7-rust-cutover-completion-report.md).

**✅ M7 cutover (2026-06-01):** the Node `packages/collector|mcp-server|cli` are **deleted** — the Rust `crates/` workspace is the sole collector + MCP server + CLI. Rust passed the full gate vs the Node reference and now **beats it**: conformance 132/132, stress 7/7, throughput ≥100% (CI 146%), p99 ~2–5ms (beats Node), RSS ~8× lower, crash-durable (Node's ring was RAM-only). Rollback: tag `node-reference-v0.10.13` (verified restorable). CI gate (`rust.yml`: build + conformance + bench-SLO) green. **Remaining = outward-facing only:** push the `v0.11.0` tag (→ npm publish + signed binaries) and `npm deprecate` the Node packages (needs npm auth) — see the completion report §6. Then → Phase SDK-Channel-Migration. The detail below is pre-cutover history.

**Active phase (history):** Phase Rust-Collector (target **v0.11.0**). **Milestone 0 complete (2026-05-29)** — Wire-Protocol-Lock shipped (v0.10.13, published to npm); Playwright→Node sidecar ([ADR-0007](./decisions/0007-playwright-node-sidecar.md)); command-channel→mcp embeds core in-process ([ADR-0008](./decisions/0008-rust-mcp-embeds-collector-core.md)); rmcp 1.7 + rusqlite dedicated-thread validated by spikes ([research 0001](./research/0001-rust-foundational-spikes.md)). **Rust port: M1 spine + M2 read API + M3 tool surface done — Rust binaries pass the full 17/17 conformance gate.** The Cargo workspace + 4 crates (`crates/`) build clean; 64 MCP tools registered across 10 family modules (M3 landed via an 8-agent Workflow fan-out). clippy clean, cargo test green.
...
| Wire-protocol + tool-shape conformance | `npm run conformance` | ✅ **132 / 132 vs Node AND Rust** — incl. the M5 pm/ workspace + API-key layer (`pm-workspaces.conformance.test.ts`, 6 tests; `PmStore` ported) and the M5.5 Slice G dev-server lifecycle (`pm-dev-server.conformance.test.ts`, 5 deterministic cases; the spawn→detect→group-kill→port-freed lifecycle is a collector-core integration test, intentionally not conformance-gated against Node's orphan bug). Audit 0002 fully closed. See [`audits/0002-rust-port-audit.md`](./audits/0002-rust-port-audit.md). |
