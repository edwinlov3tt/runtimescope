...
**⚠️ Post-audit (2026-05-29):** an external adversarial audit ([`audits/0002`](./audits/0002-rust-port-audit.md)) found the original "17/17 = equivalent" was false confidence — the gate asserted counts, not behavior. The gate is now **hardened to 33 tests** and the Rust binaries score **29/33** (Phases A+B done — Phase B closed the 8 HTTP reds: filters, `POST /api/events` ingest, timeline, 404). The 4 remaining reds — MCP tool reshaping/filters (Phase C) and auth `AUTH_FAILED`/`AUTH_TIMEOUT` error frames (Phase E) — are the rest of the backlog. Port is **blocked for launch work** until those close. Engineering practices to prevent recurrence are in [`../CLAUDE.md`](../CLAUDE.md).
...
| Wire-protocol conformance | `npm run conformance` | **33 / 33 vs Node** (gate hardened post-audit — now asserts filters/shapes/codes, not just counts). **Rust at 29/33 (Phase B done)** — the 4 remaining reds are MCP tool reshaping/filters (Phase C) + auth error frames (Phase E); Phase B closed the 8 HTTP reds. See [`audits/0002-rust-port-audit.md`](./audits/0002-rust-port-audit.md). |
