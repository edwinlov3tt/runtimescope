...
**⚠️ Post-audit (2026-05-29):** an external adversarial audit ([`audits/0002`](./audits/0002-rust-port-audit.md)) found the original "17/17 = equivalent" was false confidence — the gate asserted counts, not behavior. The gate is now **hardened to 33 tests** and the Rust binaries now pass the FULL hardened gate: **35/35 vs Rust AND Node** (Phases A+B+C-gate+E-auth/sidecar). Phase D (durability: torn-tail heal, bounded WAL, propagated write errors) + #9 (SSRF, incl. decimal/hex/IPv6) are now done too. #7 (session metadata) is now done (gated). **Only remaining: the #2/#8 tool-shape sweep** (~57 tools unverified). Green gate ≠ all findings closed. Port is **blocked for launch work** until those close. Engineering practices to prevent recurrence are in [`../CLAUDE.md`](../CLAUDE.md).
