...
| Wire-protocol + tool-shape conformance | `npm run conformance` | ✅ **116 / 116 vs Node AND Rust** — incl. the M5 pm/ workspace + API-key layer (`pm-workspaces.conformance.test.ts`, 6 tests; `PmStore` ported). Audit 0002 fully closed. See [`audits/0002-rust-port-audit.md`](./audits/0002-rust-port-audit.md). |
