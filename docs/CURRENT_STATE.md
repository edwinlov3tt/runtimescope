...
**Active phase:** Phase Rust-Collector (target **v0.11.0**). **Milestone 0 complete (2026-05-29)** — Wire-Protocol-Lock shipped (v0.10.13, published to npm); Playwright→Node sidecar ([ADR-0007](./decisions/0007-playwright-node-sidecar.md)); command-channel→mcp embeds core in-process ([ADR-0008](./decisions/0008-rust-mcp-embeds-collector-core.md)); rmcp 1.7 + rusqlite dedicated-thread validated by spikes ([research 0001](./research/0001-rust-foundational-spikes.md)). **Milestone 1 in progress** — the Cargo workspace + 4 crates (`crates/`) build clean, and the slice + persistence are green against the Rust binaries via the conformance harness: `collector-server` passes `event-roundtrip` **and `durability`** (events survive SIGKILL + restart via rusqlite WAL-mode + a JSONL WAL with fsync-before-commit, behind the dedicated-DB-owner thread — research 0001 / ADR-0008); `mcp-server` passes the `get_network_requests` round-trip (SDK → embedded in-process collector → MCP tool, ADR-0008). Remaining M1: the full 19 event types, auth (handshake-4001 / HTTP-401), WAL rotation/truncation, patterns doc (see [`roadmap/rust-collector-milestones.md`](./roadmap/rust-collector-milestones.md)). Still serial / one author until the spine is complete.
