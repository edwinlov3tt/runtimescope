...
**Active phase:** Phase Rust-Collector (target **v0.11.0**). **Milestone 0 complete (2026-05-29)** — Wire-Protocol-Lock shipped (v0.10.13, published to npm); Playwright→Node sidecar ([ADR-0007](./decisions/0007-playwright-node-sidecar.md)); command-channel→mcp embeds core in-process ([ADR-0008](./decisions/0008-rust-mcp-embeds-collector-core.md)); rmcp 1.7 + rusqlite dedicated-thread validated by spikes ([research 0001](./research/0001-rust-foundational-spikes.md)). **Milestone 1 in progress** — the Cargo workspace + 4 crates (`crates/`) build clean. The conformance gate was **broadened to 17 tests / 7 specs** (all green vs the Node source-of-truth) so it's a real done-signal, not a network-only shape-check. The **Rust binaries score 13/17** — spine + persistence + auth proven (`handshake` 3/3, `event-roundtrip` 2/2, `http-contracts` 6/6, `durability` 1/1; `mcp-tools` data-roundtrip). The 4 gaps map to remaining milestones: `event-families` → **M2** (all event types through the read API), `mcp-tool-families` + the `mcp-tools` catalog/command-channel → **M3** (the 63-tool fan-out). Remaining M1: full 19 event types, WAL rotation/truncation, patterns doc (see [`roadmap/rust-collector-milestones.md`](./roadmap/rust-collector-milestones.md)). Still serial / one author until the spine is complete. **Parallel:** the Playwright recon sidecar (ADR-0007) is built on branch `feat/recon-sidecar` / [PR #1](https://github.com/edwinlov3tt/runtimescope/pull/1).
...
| Wire-protocol conformance | `npm run conformance` | ✅ **17 / 17** (7 specs) vs the Node collector — the Rust port's acceptance gate (ADR-0006). Broadened with `event-families` (all event types → read API, gates M2) + `mcp-tool-families` (tools across families, gates M3). Rust binary currently 13/17 (the 4 gaps = M2/M3 work). |
