...
**⚠️ Post-audit (2026-05-29):** an external adversarial audit ([`audits/0002`](./audits/0002-rust-port-audit.md)) found the original "17/17 = equivalent" was false confidence — the gate asserted counts, not behavior. The gate is now **hardened to 33 tests** and the Rust binaries honestly score **21/33**; the 12 reds (HTTP filters / POST ingest / timeline / 404, MCP tool reshaping+filters, auth error frames) are the remediation backlog (Phases B–E). Port is **blocked for launch work** until those close. Engineering practices to prevent recurrence are in [`../CLAUDE.md`](../CLAUDE.md).
...
| Wire-protocol conformance | `npm run conformance` | **33 / 33 vs Node** (gate hardened post-audit — now asserts filters/shapes/codes, not just counts). **Rust honestly at 21/33** — the 12 reds are the audit-remediation backlog (HTTP filters + `POST /api/events` ingest + timeline + 404; MCP tool reshaping/filters; auth error frames). See [`audits/0002-rust-port-audit.md`](./audits/0002-rust-port-audit.md). |
