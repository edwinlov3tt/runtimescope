...
**What 17/17 means vs. doesn't:** the wire protocol, persistence/durability, auth, the full event read API, and the store-read tool surface are all proven equivalent to Node. It does **not** mean the heavy tools work — DB introspection, process/infra, `pm/` workspaces, and the Playwright-sidecar tools are registered **deferred stubs** (valid envelope, `data: null`). Making those real is the bulk of the remaining work.
...
| Wire-protocol conformance | `npm run conformance` | ✅ **17 / 17** vs Node AND **17 / 17 vs the Rust binaries** (M1–M3 conformance complete). 64 tools registered. ⚠️ Green means the wire surface + store-read tool surface match — the deferred-stub tools (DB introspection, OS/infra, pm/, scanner-sidecar) are registered but not yet functional (M4/M5). |
