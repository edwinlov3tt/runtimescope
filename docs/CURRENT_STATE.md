...
**⚠️ Post-audit (2026-05-29):** an external adversarial audit ([`audits/0002`](./audits/0002-rust-port-audit.md)) found the original "17/17 = equivalent" was false confidence — the gate asserted counts, not behavior. The gate is now **hardened to 33 tests** and the Rust binaries now pass the FULL hardened gate: **33/33 vs Rust AND 33/33 vs Node** (Phases A+B+C-gate+E-auth/sidecar). Phase D (durability: torn-tail heal, bounded WAL, propagated write errors) + #9 (SSRF, incl. decimal/hex/IPv6) are now done too. Remaining audit items: #7 (session/project metadata) + the #2/#8 tool-shape sweep (~57 tools unverified). Green gate ≠ all findings closed. Port is **blocked for launch work** until those close. Engineering practices to prevent recurrence are in [`../CLAUDE.md`](../CLAUDE.md).
...
| Wire-protocol conformance | `npm run conformance` | **33 / 33 vs Node** (gate hardened post-audit — now asserts filters/shapes/codes, not just counts). **Rust at 30/33 (Phases A+B+C-gate done)** — 3 remaining reds = auth error frames (Phase E). Phase C closed the gated get_network_requests; ~57 other tool shapes remain unverified (audit 0002). See [`audits/0002-rust-port-audit.md`](./audits/0002-rust-port-audit.md). |
