...
**⚠️ Post-audit (2026-05-29):** an external adversarial audit ([`audits/0002`](./audits/0002-rust-port-audit.md)) found the original "17/17 = equivalent" was false confidence — the gate asserted counts, not behavior. The gate is now **hardened to 33 tests** and the Rust binaries score **30/33** (Phases A+B+C-gate done — Phase B fixed HTTP, Phase C fixed get_network_requests filters/reshaping). The 3 remaining reds — auth `AUTH_FAILED`/`AUTH_TIMEOUT` error frames (Phase E). Broader #2 (other ~57 tool shapes) + #8 (catalog honesty) remain unverified — see audit 0002. Port is **blocked for launch work** until those close. Engineering practices to prevent recurrence are in [`../CLAUDE.md`](../CLAUDE.md).
