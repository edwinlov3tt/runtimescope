...
**Rust port: M1 spine + M2 read API done; M3 is next (the 63-tool fan-out — where the agent team earns its keep).** The Cargo workspace + 4 crates (`crates/collector-core|collector-server|mcp-server|cli`) build clean (axum 0.8 + rmcp 1.7 + rusqlite 0.40 + tokio). Conformance gate: 17 tests / 7 specs, all green vs the Node source-of-truth. **Rust binaries score 14/17** — spine + persistence + auth + the full event-family read API (`event-families` green via the generic `/api/events/{kind}` route; M2 collapsed to one handler because M1's Store + ingest were already type-agnostic). The 3 remaining gaps are all **M3**: `mcp-tool-families` + `mcp-tools` catalog (≥60) + command-channel. **M3 is the textbook fan-out** — ~40 store-read tools batch across agents; the ~23 heavier ones (DB introspection, engines, `pm/`, recon-via-sidecar) overlap M4/M5. Non-gating M1 tidy-up: typed event structs, WAL rotation/truncation, patterns doc. Toolchain: Rust 1.95.0. Parallel: Playwright recon sidecar on `feat/recon-sidecar` (PR #1).
