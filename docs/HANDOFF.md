...
**Milestone 1 in progress.** The Cargo workspace + 4 crates (`crates/collector-core|collector-server|mcp-server|cli`) build clean (axum 0.8 + rmcp 1.7 + rusqlite 0.40 + tokio). The conformance gate was **broadened to 17 tests / 7 specs** (all green vs the Node source-of-truth) so green = real done, not a network-only shape-check. The **Rust binaries score 13/17**: spine + persistence + auth proven (`handshake` 3/3, `event-roundtrip` 2/2, `http-contracts` 6/6, `durability` 1/1, `mcp-tools` data-roundtrip). The 4 gaps are the roadmap: `event-families` → **M2** (all event families through the read API), `mcp-tool-families` + `mcp-tools` catalog/command-channel → **M3** (the 63-tool fan-out). **Remaining M1 (serial):** full 19 serde event types, WAL rotation/truncation, patterns doc. The agent-team fan-out (M3) starts once the spine is complete. Toolchain: Rust 1.95.0. Parallel: Playwright recon sidecar on `feat/recon-sidecar` (PR #1).
