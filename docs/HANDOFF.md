...
**Milestone 1 in progress.** The Cargo workspace + 4 crates (`crates/collector-core|collector-server|mcp-server|cli`) build clean (axum 0.8 + rmcp 1.7 + rusqlite 0.40 + tokio). **13/15 conformance tests green against the Rust binaries — 4 of 5 spec files fully passing:** `handshake` 3/3 (auth 4001 close + authed accept), `event-roundtrip` 2/2, `http-contracts` 6/6 (401 gate + `/metrics`), `durability` 1/1 (rusqlite WAL + JSONL WAL fsync-before-commit, dedicated-DB-owner thread); `mcp-tools` 1/3. The 2 fails are the expected **M3** gaps: tool catalog ≥60 (only `get_network_requests` exists) + the command channel (`get_dom_snapshot`). **Remaining M1 (serial):** full 19 serde event types, WAL rotation/truncation, patterns doc. The agent-team fan-out (M3) starts once the spine is complete. Toolchain: Rust 1.95.0. Parallel: Playwright recon sidecar on `feat/recon-sidecar` (PR #1).
