...
**⚠️ Post-audit:** an external adversarial review ([`audits/0002`](./audits/0002-rust-port-audit.md)) found "17/17 = equivalent" was false confidence (the gate asserted counts, not behavior). The gate was hardened to 33 behavior-asserting tests, and **the Rust collector now passes the FULL gate: 33/33 vs Rust AND 33/33 vs Node** (Phases A gate + B HTTP + C-gate tool shapes + E auth/sidecar). **But green gate ≠ all findings closed** — the remaining audit items are NON-gated: **Phase D** (durability #3/#4/#5 — torn-tail truncate, WAL rotation, propagate write errors), **#7** (separate projectName/projectId + persist sessions), and the **#2/#8 tool-shape sweep** (~57 agent-ported tools still unverified; deferred stubs still count toward the catalog). Still **blocked for launch** until those close. Practices to prevent recurrence: [`../CLAUDE.md`](../CLAUDE.md) → "Engineering practices & review discipline".
