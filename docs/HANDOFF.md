...
**Milestone 1 in progress.** The Cargo workspace + 4 crates (`crates/collector-core|collector-server|mcp-server|cli`) build clean (axum 0.8 + rmcp 1.7 + tokio), and the **vertical slice is GREEN against the Rust binaries** via the conformance harness — `collector-server` passes `event-roundtrip` and `mcp-server` passes the `get_network_requests` round-trip (the ADR-0008 embed-in-process topology working). **Remaining M1 (still serial, one author):** rusqlite+WAL persistence behind the dedicated-DB-owner thread, the full 19 serde event types, auth (handshake-4001 / HTTP-401), and a short patterns doc. The agent-team fan-out (M3) does not start until the spine is complete. Toolchain: Rust 1.95.0.
