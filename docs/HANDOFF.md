...
**17/17 means the wire protocol + persistence + auth + read API + store-read tool surface are proven equivalent to Node. It does NOT mean the heavy tools work** — DB introspection, process/infra, `pm/` workspaces, and the Playwright-sidecar tools are registered **deferred stubs** (`data: null`). Making them real is the remaining work: **M4** (deepen the stubs — DB introspection engine, OS/infra, recon-via-sidecar), **M5** (`pm/` subsystem ~4.4K LOC), **M1 tidy** (WAL rotation, typed events), **M6** (cli + curl-install + dashboard embed), **M7** (cutover + ship v0.11.0). Toolchain: Rust 1.95.0. Parallel: Playwright recon sidecar on `feat/recon-sidecar` (PR #1) — feeds M4's scanner tools.
