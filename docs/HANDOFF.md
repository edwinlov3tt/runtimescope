...
**⚠️ Post-audit:** an external adversarial review ([`audits/0002`](./audits/0002-rust-port-audit.md)) found "17/17 = equivalent" was false confidence (the gate asserted counts, not behavior). **The gate is now hardened to 33 tests; Rust honestly scores 21/33.** The 12 reds are the remediation backlog: HTTP query filters + `POST /api/events` ingest (Workers/Python SDKs depend on it) + `timeline` merge + unknown-route 404; the MCP tool reshaping/filters; and auth `AUTH_FAILED`/`AUTH_TIMEOUT` error frames. **Blocked for launch until Phases B–E close them.** Phase A (gate hardening) is done. Practices to prevent recurrence: [`../CLAUDE.md`](../CLAUDE.md) → "Engineering practices & review discipline".
