...
**⚠️ Post-audit:** an external adversarial review ([`audits/0002`](./audits/0002-rust-port-audit.md)) found "17/17 = equivalent" was false confidence (the gate asserted counts, not behavior). **The gate is hardened to 33 tests; Rust scores 29/33 (Phases A+B done).** Phase B closed the 8 HTTP reds (filters, POST ingest, timeline, 404). The 4 remaining: MCP tool reshaping/filters (Phase C), auth AUTH_FAILED/AUTH_TIMEOUT error frames (Phase E). **Blocked for launch until Phases B–E close them.** Phase A (gate hardening) is done. Practices to prevent recurrence: [`../CLAUDE.md`](../CLAUDE.md) → "Engineering practices & review discipline".
