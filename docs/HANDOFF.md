...
**⚠️ Post-audit:** an external adversarial review ([`audits/0002`](./audits/0002-rust-port-audit.md)) found "17/17 = equivalent" was false confidence (the gate asserted counts, not behavior). **The gate is hardened to 33 tests; Rust scores 30/33 (Phases A+B+C-gate done).** Phase B closed the 8 HTTP reds (filters, POST ingest, timeline, 404). The 3 remaining reds: auth AUTH_FAILED/AUTH_TIMEOUT error frames (Phase E). (Phase C closed the gated get_network_requests; the other ~57 tool shapes remain unverified — see audit 0002 Phase C note.) **Blocked for launch until Phases B–E close them.** Phase A (gate hardening) is done. Practices to prevent recurrence: [`../CLAUDE.md`](../CLAUDE.md) → "Engineering practices & review discipline".
