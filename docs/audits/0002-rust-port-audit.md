...
**Status:** Open — **conformance gate GREEN at 35/35 vs Node AND Rust.** Done: A (gate), B (HTTP), C-gate, D (durability), E (auth/sidecar), #7 (session metadata), #9 (SSRF). **Only remaining: the #2/#8 tool-shape sweep** (~57 agent-ported tools still unverified; deferred stubs still count toward the catalog).
**Date:** 2026-05-29
