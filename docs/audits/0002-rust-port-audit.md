...
**Status:** Open — **conformance gate fully GREEN: 33/33 vs Node AND 33/33 vs Rust** (Phases A + B + C-gate + E-auth/sidecar done). Remaining (all NON-gated by the current suite): **Phase D** (durability #3/#4/#5), **#7** (session/project metadata), and the **#2/#8 tool-shape sweep** (~57 tools unverified). Green gate ≠ all findings closed — these four are not yet covered.
**Date:** 2026-05-29
...
- **Phase E — auth + sidecar DONE; metadata (#7) pending.** #6: token compare is now **constant-time** (`subtle::ct_eq`), and the WS auth gate emits distinct `AUTH_FAILED` (bad/missing token) vs `AUTH_TIMEOUT` (no handshake) error frames **before** the 4001 close — `auth-frames` green vs Rust (closes the reconnect storm). #9: `RUNTIMESCOPE_RECON_SIDECAR` accepts a JSON argv array (paths with spaces); `scan_website` has an SSRF guard (http/https only; rejects localhost/127./10./192.168./172.16-31./169.254./`.local`/`.internal`/`file://`) — verified. **#7 (separate projectName/projectId + persist sessions) NOT done** — a schema/rehydration change, deferred to its own increment (non-gated; affects post-restart session/project metadata).
