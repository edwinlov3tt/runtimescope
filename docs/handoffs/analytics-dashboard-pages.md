...
| `analytics-compare.html` | `analytics-compare` | `GET /api/analytics/compare?by=role` and `?by=app` (value/prevValue $); `GET /api/analytics/narrative?by=role\|app` (the insight line — **implemented collector-side**, no Mosaic needed) | richer mc-narrative templating is the future Mosaic option |
