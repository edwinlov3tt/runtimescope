...
- [x] v0.11.0; deprecate Node packages (final v0.10.13) on npm; completion report; CURRENT_STATE + HANDOFF → Phase SDK-Channel-Migration. **🎉 v0.11.0 SHIPPED (tag `v0.11.0`, 2026-06-02). All channels live + CI-green: npm SDKs + integrations @0.11.0, crates.io (`cargo install runtimescope` → core/mcp/runtimescope, dashboard embedded), GitHub Release (universal macOS binaries + SHA256SUMS), PyPI (Python SDK). Two CI fixes during the cutover: `cargo publish --allow-dirty` for the vendored dashboard; publish.yml moved to macos-14 to match the lockfile (rollup native-dep, npm/cli#4828). REMAINING (operator, needs `npm login`): `npm deprecate` the Node packages — report §6.**
