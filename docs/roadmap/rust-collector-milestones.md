...
- [~] Port order: pm-types → pm-store (SQLite schema + CRUD) → session-parser → project-discovery → pm-routes → wire the 4 MCP workspace tools + the workspace-API-key auth path.
  ...
  - **Remaining M5 (fast-follow / M6-M7 cleanup):** RuntimeScope-project discovery (needs the ProjectManager port) + capex stub; the deferred pm-routes write CRUD (tasks/notes/memory/api-keys); the auth-enablement reconciliation above. **M5 core is functionally complete.**
