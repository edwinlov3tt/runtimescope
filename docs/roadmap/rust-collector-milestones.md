...
- [~] Port order: pm-types → pm-store (SQLite schema + CRUD) → session-parser → project-discovery → pm-routes → wire the 4 MCP workspace tools + the workspace-API-key auth path.
  - [x] **Workspace + API-key characterization gate written** — `pm-workspaces.conformance.test.ts` (6 tests) pins `list_workspaces` (auto-"Personal"), `create_workspace` (slug derivation + dup rejection), `create_workspace_api_key` (`tk_`+48hex secret, prefix/last4, key-count bump, unknown-ws error), `move_project_to_workspace` not-found. **Green vs Node; RED vs Rust (68/74)** — the 6 reds are the workspace-tool port backlog. Next: port pm-store workspace/apikey (SQLite `pm_workspaces`/`pm_api_keys`, `ws_`/`tk_` gen, SHA-256 key hash, auto-"Personal") + wire the 4 tools to turn them green.
