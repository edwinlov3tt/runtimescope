...
- [ ] **⚠️ Cross-cutting finding — FK enforcement divergence (handle in a dedicated pass):** `better-sqlite3` defaults `PRAGMA foreign_keys = ON`, so Node's pm.db **enforces** every declared FK at runtime (`pm_tasks/pm_notes/pm_capex/pm_sessions → pm_projects`, `pm_api_keys → pm_workspaces ON DELETE CASCADE`). `rusqlite` defaults FK **OFF**, so Rust currently accepts rows with dangling `project_id`/`session_id` where Node 400s. (This **corrects** the earlier "FKs are inert in both" note — they're inert in *neither* at the Node runtime.) Faithful fix = enable the pragma on the Rust pm.db + add the FK constraints + propagate create-insert errors to a 400. **Deferred deliberately**: it touches every pm table AND the live `~/.claude` discovery insert path (must verify parent-before-child ordering so FK-on doesn't break indexing), so it warrants its own verified pass, not a per-slice flip. The Slice-A/B/C conformance specs avoid crossing the FK boundary in the meantime (tasks/notes use NULL `project_id`; capex isn't HTTP-created).
