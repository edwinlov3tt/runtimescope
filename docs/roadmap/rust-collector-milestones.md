...
- [~] Port order: pm-types → pm-store (SQLite schema + CRUD) → session-parser → project-discovery → pm-routes → wire the 4 MCP workspace tools + the workspace-API-key auth path.
  ...
  - **Remaining M5:** RuntimeScope-project discovery (needs the ProjectManager port) + capex stub; then **pm-routes (`/api/pm/*`)** → **workspace-API-key auth path** in the WS handshake.
